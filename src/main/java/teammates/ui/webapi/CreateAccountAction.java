...
{
throw new  EntityNotFoundException("Account request with registration key " + registrationKey + " could not be found");
}
...
try {
logic.updateAccountRequest(AccountRequestAttributes.updateOptionsBuilder(instructorEmail, instructorInstitution).withRegisteredAt(Instant.now()).build());
}
...
private  String importDemoData(String instructorEmail, String instructorName, String instructorInstitute, String timezone)throws InvalidParametersException {
...
String dataBundleString = Templates.populateTemplate(Templates.INSTRUCTOR_SAMPLE_DATA, "teammates.demo.instructor@demo.course", instructorEmail, "Demo_Instructor", instructorName, "demo.course", courseId, "demo.institute", instructorInstitute, "demo.timezone", timezone, "demo.date1", dateString1, "demo.date2", dateString2, "demo.date3", dateString3, "demo.date4", dateString4, "demo.date5", dateString5);
...
return ZonedDateTime.ofInstant(instant, ZoneId.of(Const.DEFAULT_TIME_ZONE)).withZoneSameLocal(timezone).toInstant().toString();
}
);
}
