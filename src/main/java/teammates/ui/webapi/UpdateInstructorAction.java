...
InstructorAttributes instructorToEdit = retrieveEditedInstructor(courseId, instructorRequest.getId(), instructorRequest.getName(), instructorRequest.getEmail(), instructorRequest.getRoleName(), instructorRequest.getIsDisplayedToStudent(), instructorRequest.getDisplayName());
...
{
updatedInstructor = logic.updateInstructor(InstructorAttributes.updateOptionsWithEmailBuilder(instructorToEdit.getCourseId(), instructorRequest.getEmail()).withName(instructorToEdit.getName()).withDisplayedName(instructorToEdit.getDisplayedName()).withIsDisplayedToStudents(instructorToEdit.isDisplayedToStudents()).withRole(instructorToEdit.getRole()).withPrivileges(instructorToEdit.getPrivileges()).build());
}
else
{
updatedInstructor = logic.updateInstructorCascade(InstructorAttributes.updateOptionsWithGoogleIdBuilder(instructorToEdit.getCourseId(), instructorRequest.getId()).withEmail(instructorToEdit.getEmail()).withName(instructorToEdit.getName()).withDisplayedName(instructorToEdit.getDisplayedName()).withIsDisplayedToStudents(instructorToEdit.isDisplayedToStudents()).withRole(instructorToEdit.getRole()).withPrivileges(instructorToEdit.getPrivileges()).build());
}
...
private  InstructorAttributes retrieveEditedInstructor(String courseId, String instructorId, String instructorName, String instructorEmail, String instructorRole, boolean isDisplayedToStudents, String displayedName) {
