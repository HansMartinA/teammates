...
QueryLogsParams queryLogsParams = QueryLogsParams.builder(startTime, endTime).withSeverityLevel(severity).withMinSeverity(minSeverity).withTraceId(traceId).withActionClass(actionClass).withUserInfo(userInfoParams).withLogEvent(logEvent).withSourceLocation(new  SourceLocation(sourceLocationFile, null, sourceLocationFunction)).withExceptionClass(exceptionClass).withLatency(latency).withStatus(status).withVersion(version).withExtraFilters(extraFilters).withOrder(order).withPageSize(DEFAULT_PAGE_SIZE).build();
