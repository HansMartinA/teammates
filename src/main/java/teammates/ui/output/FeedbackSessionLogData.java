...
public  FeedbackSessionLogData(FeedbackSessionAttributes feedbackSession, List<FeedbackSessionLogEntry> logEntries, Map<String, StudentAttributes> studentsMap){
FeedbackSessionData fsData = new  FeedbackSessionData(feedbackSession);
List<FeedbackSessionLogEntryData> fsLogEntryDatas = logEntries.stream().map(log -> new  FeedbackSessionLogEntryData(log, studentsMap.get(log.getStudentEmail()))).collect(Collectors.toList());
