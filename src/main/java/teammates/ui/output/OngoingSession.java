...
if (account != null)
{
instructorHomePageLink = Config.getFrontEndAppUrl(Const.WebPageURIs.INSTRUCTOR_HOME_PAGE).withUserId(account.getGoogleId()).toString();
}
this.instructorHomePageLink = instructorHomePageLink;
this.startTime = TimeHelper.getMidnightAdjustedInstantBasedOnZone(fs.getStartTime(), fs.getTimeZone(), true).toEpochMilli();
this.endTime = TimeHelper.getMidnightAdjustedInstantBasedOnZone(fs.getEndTime(), fs.getTimeZone(), true).toEpochMilli();
