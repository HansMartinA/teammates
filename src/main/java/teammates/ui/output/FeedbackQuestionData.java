...
this.customNumberOfEntitiesToGiveFeedbackTo = feedbackQuestionAttributes.getNumberOfEntitiesToGiveFeedbackTo();
}
...
this.showRecipientNameTo = convertToFeedbackVisibilityType(feedbackQuestionAttributes.getShowRecipientNameTo());
if (this.questionType == FeedbackQuestionType.CONTRIB && this.giverType == FeedbackParticipantType.STUDENTS && this.recipientType == FeedbackParticipantType.OWN_TEAM_MEMBERS_INCLUDING_SELF && this.showResponsesTo.contains(FeedbackVisibilityType.GIVER_TEAM_MEMBERS))
{
...
{
FeedbackConstantSumQuestionDetails constantSumQuestionDetails = (FeedbackConstantSumQuestionDetails) this.questionDetails;
this.questionType = constantSumQuestionDetails.isDistributeToRecipients() ? FeedbackQuestionType.CONSTSUM_RECIPIENTS : FeedbackQuestionType.CONSTSUM_OPTIONS;
this.questionDetails.setQuestionType(this.questionType);
}
}
private  List<FeedbackVisibilityType> convertToFeedbackVisibilityType(List<FeedbackParticipantType> feedbackParticipantTypeList) {
return feedbackParticipantTypeList.stream().map(feedbackParticipantType -> {
switch (feedbackParticipantType) {
case STUDENTS: return FeedbackVisibilityType.STUDENTS;
case INSTRUCTORS: return FeedbackVisibilityType.INSTRUCTORS;
case RECEIVER: return FeedbackVisibilityType.RECIPIENT;
case OWN_TEAM_MEMBERS: return FeedbackVisibilityType.GIVER_TEAM_MEMBERS;
case RECEIVER_TEAM_MEMBERS: return FeedbackVisibilityType.RECIPIENT_TEAM_MEMBERS;
default: assert false : "Unknown feedbackParticipantType" + feedbackParticipantType;
break;
...
if (questionDetails instanceof FeedbackRubricQuestionDetails)
{
((FeedbackRubricQuestionDetails) questionDetails).setRubricWeightsForEachCell(Collections.singletonList(Collections.emptyList()));
}
}
