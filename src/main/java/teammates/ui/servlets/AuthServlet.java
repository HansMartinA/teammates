...
return new  GoogleAuthorizationCodeFlow.Builder(HTTP_TRANSPORT, JSON_FACTORY, Config.OAUTH2_CLIENT_ID, Config.OAUTH2_CLIENT_SECRET, SCOPES).setDataStoreFactory(DATA_STORE_FACTORY).setAccessType("offline").build();
}
...
Cookie cookie = new  Cookie(Const.SecurityConfig.AUTH_COOKIE_NAME, StringHelper.encrypt(JsonUtils.toCompactJson(uic)));
cookie.setPath("/");
cookie.setSecure(!Config.isDevServer());
cookie.setHttpOnly(true);
cookie.setMaxAge((int) Const.COOKIE_VALIDITY_PERIOD.toSeconds());
return cookie;
}
