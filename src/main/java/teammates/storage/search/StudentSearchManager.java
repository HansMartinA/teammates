...
result.sort(Comparator.comparing((StudentAttributes student) -> student.getCourse()).thenComparing(student -> student.getSection()).thenComparing(student -> student.getTeam()).thenComparing(student -> student.getName()).thenComparing(student -> student.getEmail()));
}
...
{
courseIdsWithViewStudentPrivilege = instructors.stream().filter(i -> i.getPrivileges().getCourseLevelPrivileges().isCanViewStudentInSections()).map(ins -> ins.getCourseId()).collect(Collectors.toList());
...
List<SolrDocument> filteredDocuments = documents.stream().filter(document -> {
if (instructors == null)
{
return true;
}
