...
QueryLogsParams queryLogsParams = QueryLogsParams.builder(startTime.toEpochMilli(), endTime.toEpochMilli()).withMinSeverity(LogSeverity.ERROR).build();
LogSearchParams logSearchParams = LogSearchParams.from(queryLogsParams).addLogName(REQUEST_LOG_NAME).setResourceType(RESOURCE_TYPE_GAE_APP);
...
JsonFormat.TypeRegistry tr = JsonFormat.TypeRegistry.newBuilder().add(RequestLog.getDescriptor()).add(LogLine.getDescriptor()).add(com.google.appengine.logging.v1.SourceLocation.getDescriptor()).add(SourceReference.getDescriptor()).build();
...
{
errorLogs.add(new  ErrorLogEntry(line.getLogMessage().replaceAll("\n", "\n<br>"), line.getSeverity().toString(), trace));
}
}
}
return errorLogs;
...
LogSearchParams logSearchParams = LogSearchParams.from(queryLogsParams).addLogName(STDOUT_LOG_NAME).addLogName(STDERR_LOG_NAME).setResourceType(RESOURCE_TYPE_GAE_APP);
...
GeneralLogEntry logEntry = new  GeneralLogEntry(convertSeverity(severity), trace, insertId, resourceIdentifier, new  SourceLocation(file, line, function), timestamp);
...
QueryLogsParams queryLogsParams = QueryLogsParams.builder(startTime, endTime).withLogEvent(LogEvent.FEEDBACK_SESSION_AUDIT.name()).withSeverityLevel(LogSeverity.INFO).withExtraFilters(String.join("\n", filters)).build();
LogSearchParams logSearchParams = LogSearchParams.from(queryLogsParams).addLogName(STDOUT_LOG_NAME).setResourceType(RESOURCE_TYPE_GAE_APP);
Page<LogEntry> entries = getLogEntries(logSearchParams, 0);
List<LogEntry> logEntries = new  ArrayList<>();
for (LogEntry entry : entries.iterateAll())
{
...
FeedbackSessionLogEntry fslEntry = new  FeedbackSessionLogEntry(details.getStudentEmail(), details.getFeedbackSessionName(), details.getAccessType(), timestamp);
...
{
String logNameFilter = s.logName.stream().map(str -> "\"projects/" + options.getProjectId() + "/logs/" + str + "\"").collect(Collectors.joining(" OR "));
...
if (q.getMinSeverity() != null && q.getSeverity() == null)
{
...
{
logFilters.add("sourceLocation.file=\"" + q.getSourceLocation().getFile() + "\" AND sourceLocation.function=\"" + q.getSourceLocation().getFunction() + "\"");
}
}
