...
log.fine("normalizedAveragePerceivedAsDouble as double:\n" + replaceMagicNumbers(Arrays.toString(normalizedAveragePerceivedAsDouble)));
...
denormalizedAveragePerceived = calculatePerceivedForStudents(claimedSanitized, normalizedAveragePerceivedAsDouble);
...
private static  double[] []  adjustPeerContributionRatioToTallyNormalizedAveragePerceived(double[] []  peerContributionRatio) {
...
private  int[] []  calculatePerceivedForStudents(int[] []  claimedSanitized, double[]  normalizedAveragePerceivedAsDouble) {
int teamSize = claimedSanitized.length;
int[] []  output = new int [teamSize] [teamSize] ;
for (int k = 0 ; k < teamSize ; k++)
{
output[k] = calculatePerceivedForStudent(claimedSanitized[k], normalizedAveragePerceivedAsDouble);
}
return output;
}

static  int[]  calculatePerceivedForStudent(int[]  claimedSanitizedRow, double[]  normalizedAveragePerceivedAsDouble) {
verify("Unsanitized value received ", isSanitized(claimedSanitizedRow));
double[]  filteredPerceived = purgeValuesCorrespondingToSpecialValuesInFilter(intToDouble(claimedSanitizedRow), normalizedAveragePerceivedAsDouble);
int[]  filteredSanitizedActual = doubleToInt(purgeValuesCorrespondingToSpecialValuesInFilter(normalizedAveragePerceivedAsDouble, intToDouble(claimedSanitizedRow)));
...
return doubleToInt(multiplyByFactor(factor, normalizedAveragePerceivedAsDouble));
}
...
static  double[]  purgeValuesCorrespondingToSpecialValuesInFilter(double[]  filterArray, double[]  valueArray) {
...
verify("Unsanitized value in " + Arrays.toString(input), isSanitized(doubleToInt(input)));
double sum = NA;
...
double idealSum = count * Const.POINTS_EQUAL_SHARE * 1.0D;
...
for (int i = 0 ; i < input.length ; i++)
{
verify("Unsanitized value in " + Arrays.toString(input[i]), isSanitized(doubleToInt(input[i])));
...
{
returnValue.append("=======================").append(System.lineSeparator());
}
}
...
return returnValue.replace(NA + ".0", " NA").replace(Integer.toString(NA), " NA").replace(NSB + ".0", "NSB").replace(NSU + ".0", "NSU");
}
...
String filler = "                                ";
sb.append(indentString).append(pointsToString(claimed).replace(System.lineSeparator(), System.lineSeparator() + indentString + filler)).append(divider).append("              normalizedClaimed:").append(indentString).append(pointsToString(normalizedClaimed).replace(System.lineSeparator(), System.lineSeparator() + indentString + filler)).append(divider).append("normalizedPeerContributionRatio:").append(indentString).append(pointsToString(normalizedPeerContributionRatio).replace(System.lineSeparator(), System.lineSeparator() + indentString + filler)).append(divider).append("     normalizedAveragePerceived:").append(indentString).append(pointsToString(normalizedAveragePerceived).replace(System.lineSeparator(), System.lineSeparator() + indentString + filler)).append(divider).append("   denormalizedAveragePerceived:").append(indentString).append(pointsToString(denormalizedAveragePerceived).replace(System.lineSeparator(), System.lineSeparator() + indentString + filler)).append(divider);
return sb.toString();
}
